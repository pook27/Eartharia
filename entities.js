...
class Entity {
    ...
    hit(dmg, dir) {
        ...
        if (this instanceof Player) {
            spawnFloatText(this.x, this.y - 20, `-${finalDmg}`, '#f44336');
        }
...
class Player extends Entity {
    constructor(x, y) {
        ...
        this.hp = 100;
        this.maxHp = 100;
        // Expanded inventory to 50 slots
        this.inv = new Array(50).fill(null).map(() => ({id: 0, n: 0}));
        // Armor slots: 0=Head, 1=Chest, 2=Legs
        this.armor = [null, null, null];
        this.defense = 0;
        this.sel = 0; // Hotbar selection (0-9)
        this.regenTimer = 0;

        this.inv[0] = { id: IDS.PICK_WOOD, n: 1 };
        this.inv[1] = { id: IDS.AXE_WOOD, n: 1 };
        this.inv[2] = { id: IDS.SWORD_WOOD, n: 1 };
    ...
    update() {
        ...
        for(let piece of this.armor) {
            if(piece && PROPS[piece.id]) {
                this.defense += PROPS[piece.id].defense || 0;
            }
        }
        ...
        if (!uiOpen && !isInventoryOpen) {
            ...
            if ((keys['w'] || keys['ArrowUp'] || keys[' ']) && this.ground) {
                this.vy = -9.5;
            }
        } else {
            this.vx *= 0.82;
        ...
        this.regenTimer++;
        if (this.regenTimer >= 100 && this.hp < this.maxHp) {
            this.hp++;
            this.regenTimer = 0;
        }
    }
    ...
    has(id, n) {
        return this.inv.some(s => s.id === id && s.n >= n);
    }
    ...
    use(slot) {
        ...
        if (prop.consumable) {
            if (item.id === IDS.POTION && this.hp < this.maxHp) {
...
class Slime extends Entity {
    ...
    die() {
        ...
        if (Math.random() > 0.6) spawnLoot(this.x + this.w/2, this.y + this.h/2, IDS.COIN, this.big ? 3 : 1);
        if (Math.random() > 0.85 && this.big) spawnLoot(this.x + this.w/2, this.y + this.h/2, IDS.HEART, 1);
    }
}
...
class Zombie extends Entity {
    ...
    die() {
        super.die();
        spawnLoot(this.x + this.w/2, this.y + this.h/2, IDS.COIN, 3 + Math.floor(Math.random() * 5));
        if (Math.random() > 0.7) spawnLoot(this.x + this.w/2, this.y + this.h/2, IDS.HEART, 1);
    }
}
...
class FallenStar extends Entity {
    ...
    update() {
        ...
        // Trail
        if (Math.random() > 0.5) {
            spawnParticles(this.x + 8, this.y + 8, '#ffeb3b', 1);
        }
    }

    die() {
        this.dead = true;
        spawnLoot(this.x, this.y - 16, IDS.STAR, 1);
        spawnParticles(this.x, this.y, '#ffd700', 8);
        // Maybe small explosion effect?
    }
}
